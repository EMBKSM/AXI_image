...
(
    ...
    // Buffer -> Sobel Core
    wire [C_S00_AXIS_TDATA_WIDTH-1:0] buf_data;
    wire buf_valid, buf_ready;

    // Sobel Core -> Master
    wire [C_M00_AXIS_TDATA_WIDTH-1:0] sobel_data;
    wire sobel_valid, sobel_ready;

    // 1. AXI Lite Instantiation
    ...
    skid_buffer # (
        ...
        .m_data(buf_data),
        .m_strb(), // Unused
        .m_last()  // Ignore Last (continuous stream)
    );

    // 4. Sobel Core
    sobel_core # (
        .DATA_WIDTH(C_S00_AXIS_TDATA_WIDTH),
        .IMG_WIDTH(32) // Fixed width 32
    ) sobel_inst (
        .aclk(s00_axis_aclk),
        .aresetn(s00_axis_aresetn),
    ...
    my_image_ip_v1_0_M00_AXIS # ( 
        ...
        .last_in(1'b0),     // TLAST 0 (continuous stream)
        .ready_out(sobel_ready)
    );
